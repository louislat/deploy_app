...
base_longlat["CP"] = base_longlat["CP"].astype(str)
base_longlat = base_longlat.groupby("CP").agg(np.mean)
BASE_PRODUITS = BASE_PRODUITS.merge(base_longlat, how="left",on="CP")
...
def carte_panier(choix_panier):
    df1 = Base_pan.copy()
    df1 = df1[["Adresse","CP"]+[produit for produit in choix_panier]]
    df1["CP"] = df1["CP"].astype(str)
    df1["Total_panier"] = df1.sum(axis=1)
    df1 = df1.merge(base_longlat, how="left",on="CP")
    carte = px.scatter_mapbox(df1, lat="latitude_degre", lon="longitude_degre",zoom=4,color="Total_panier")
